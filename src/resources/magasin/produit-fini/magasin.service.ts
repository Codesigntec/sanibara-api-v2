...
export class MagasinService {
    ...
    list = async (filter: MagasinFetcher, query: PaginationQuery, userId: string): Promise<Pagination<Magasin>> => {
        ...
        if (magasinIdsProduitFinis.length === 0 && utilisateur.role.libelle !== 'Administrateur') {
            return {
                data: [],
                totalPages: 0,
                totalCount: 0,
        ...
        const totalCount = await this.db.magasinProduitFini.count({ where: conditions });
    ...
    save = async (data: MagasinSaver, userId: string): Promise<Magasin> => {
        const check = await this.db.magasinProduitFini.findFirst({
            where: {
                nom: {
                    equals: data.nom,
                    mode: 'insensitive'
        ...
        const magasin = await this.db.magasinProduitFini.create({
            data: {
                nom: data.nom,
        ...
        const description = `Ajout de magasin des produits finis: ${data.nom}`
    ...
    update = async (id: string, data: MagasinSaver, userId: string): Promise<Magasin> => {
        const check = await this.db.magasinProduitFini.findUnique({ where: { id: id }, select: { nom: true } })
        if (!check) throw new HttpException(errors.NOT_EXIST, HttpStatus.BAD_REQUEST);

        const checkFirst = await this.db.magasinProduitFini.findFirst({
            where: {
                id: {
                    not: id
                },
        ...
        const description = `Modification du magasin des produits finis: ${check.nom} -> ${data.nom}`
    ...
    archive = async (id: string, userId: string): Promise<Magasin> => {
        const check = await this.db.magasinProduitFini.findUnique({ where: { id: id }, select: { nom: true, archive: true } })
        if (!check) throw new HttpException(errors.NOT_EXIST, HttpStatus.BAD_REQUEST);
        ...
        const description = `Archivage du magasin des produits finis: ${check.nom}`
    ...
    remove = async (id: string, userId: string): Promise<Magasin> => {
        const check = await this.db.magasinProduitFini.findUnique({ where: { id: id }, select: { nom: true, removed: true } })
        if (!check) throw new HttpException(errors.NOT_EXIST, HttpStatus.BAD_REQUEST);
        ...
        const description = `Suppression logique du magasin des produits finis: ${check.nom}`
    ...
    destroy = async (id: string, userId: string): Promise<Magasin> => {
        ...
        try {
            const magasin = await this.db.magasinProduitFini.delete({
                where: { id },
                select: { id: true, numero: true, nom: true, adresse: true, createdAt: true }
            })

            const description = `Suppression physique du magasin des produits finis: ${check.nom}`
